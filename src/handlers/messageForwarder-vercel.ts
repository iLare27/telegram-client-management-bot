/**
 * Message forwarding handler (Vercel version)
 * Uses Vercel Postgres instead of TypeORM
 */

import { Context } from "grammy";
import { config } from "../config.js";
import { findClientTopic, findClientTopicByTopicId } from "../database/vercel-storage.js";
import { messages } from "../messages/messages.js";

/**
 * Forward messages from client to their topic in the group
 */
export async function forwardClientMessageVercel(ctx: Context): Promise<void> {
  ...
  try {
    const clientTopic = await findClientTopic(userId);
 ...
 */
export async function forwardTeamMessageVercel(ctx: Context): Promise<void> {
  ...
  try {
    ...
    const clientTopic = await findClientTopicByTopicId(topicId);
